...
const CLIENT_ID = process.env.CLIENT_ID || "antifraud-service";
const GROUP_ID = process.env.GROUP_ID || "antifraud-group";
...
const subscribe = async (
    ...
    await consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
            if (topic !== "TransactionEvents") {
                return;
            }

            if (message.key && message.value) {
                const inputMessage: MessageType = {
                    event: message.key.toString() as TransactionEvent,
