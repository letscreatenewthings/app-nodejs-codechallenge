...
import { MessageType, AntifraudEvent, TOPIC_TYPE } from "../../types";

// configuration properties
const CLIENT_ID = process.env.CLIENT_ID || "transaction-service";
const GROUP_ID = process.env.GROUP_ID || "transaction-group";
...
const connectProducer = async <T>(): Promise<T> => {
    await createTopic(["TransactionEvents"]);
...
const subscribe = async (
    ...
    await consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
            if (topic !== "AntifraudEvents") {
                return;
            }

            if (message.key && message.value) {
                const inputMessage: MessageType = {
                    event: message.key.toString() as AntifraudEvent,
